...
class TreeGraph:
    ...
    def hierarchy_pos(self):
        ...
        Returns the hierarchy positions for a vertical tree or a horizontal tree.
        '''
        ...
        def _hierarchy_pos(G, root, spread, gap, root_x, root_y, pos=None, parent=None):
            ...
            if len(children)!=0:
                sp = spread/len(children) #sp divides the spread in all children
...
# G.node_traces(Gpos,root_trace,node_trace)

# # for node, adjacencies in enumerate(G.nx_Graph().adjacency()):
# #     node_trace['marker']['color']+=tuple([len(adjacencies[1])])
# #     node_info = '# of connections: '+str(len(adjacencies[1]))

# annotations = [] #List of edge abel settings
# for edge in G.nx_Graph().edges(): 
#     label = G.edge_midpoint(Gpos[edge[0]],Gpos[edge[1]])
#     annotations.append(dict(x=label[0],y=label[1],xref='x',yref='y',text=G.edge_lbls()[edge],
#                            showarrow=True,borderpad=1,align='center',ax=3,ay=-5,arrowsize=1,arrowwidth=1,
#                            font=dict(size=11,color='#333')))

# layoutDefaults = G.get_PlotLayoutAxisInf()
...
# import plotly.io as pio #Save image file
